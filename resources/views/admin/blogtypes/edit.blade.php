...
@section('breadcrumb-second','Sửa loại tin')
@section('main')
<a href="{{ route('loai-blog.index') }}"><i class="fa fa-angle-double-left"></i>&nbsp; Quay lại</a>
<div class="row">
    <div class="col-12">
        <div class="card-box">
            <form action="{{route('loai-blog.update',$row->id)}}" method="POST">
               {{csrf_field()}}
                {!!method_field('patch')!!}
                <div class="form-group row">
                    <label for="example-text-input" class="col-2 col-form-label">Tên loại tin</label>

                    <div class="col-10">
                        <input class="form-control" type="text" value="{{ $row->title }}" id="example-text-input" name="title"
                            placeholder="Nhập tên loại tin...">
                        @error('title')
                ...
                <div class="form-group row">
                    <label for="example-text-input" class="col-2 col-form-label">Mô tả</label>

                    <div class="col-10">
                        <textarea class="form-control" type="text" id="example-text-input" name="description"
                            placeholder="Nhập mô tả...">{{$row->description}}</textarea>
                        @error('description')
                        <div style="color:red" class="ml-1">
                            <i class="typcn typcn-delete"></i> {{ $message }}
                        </div>
